...
function Campaigns(searchdata) {
  ...
  const fetchBanners = async (searchTerm = "") => {
    ...
    try {
      ...
      if (searchTerm) {
        response = await axios.get(`${API_BASE_URL}/banners/banner-search?search=${searchTerm}`);
      } else {
        // Otherwise, make the normal request to fetch all banners
        response = await axios.get(`${API_BASE_URL}/banners`);
      }
  ...
  const fetchDesktopBanners = async (searchTerm = "") => {
    ...
    try {
      const response = await axios.get(
        `${API_BASE_URL}/desktop-banners/?search=${searchTerm}`
      );
  ...
  const fetchMobileBanners = async (searchTerm = "") => {
    ...
    try {
      const response = await axios.get(
        `${API_BASE_URL}/mobile-banners/?search=${searchTerm}`
      );
      // console.log(response.data);
  ...
  const toggleHandler = async (tab, bannerId, bannerStatus) => {
    ...
    const toggleApiBasedOnTab = () => {
      switch (tab) {
        case "home":
          return `${API_BASE_URL}/banners/update-status/${bannerId}`;
        case "desktop":
          return `${API_BASE_URL}/desktop-banners/update-status/${bannerId}`;
        case "mobile":
          return `${API_BASE_URL}/mobile-banners/update-status/${bannerId}`;
        default:
          return "";
      }
    };
