...
function EditCourseForm1({ courseId }) {
  ...
  const handleSearch = async (courseId) => {
    ...
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/courses/course/${courseId}`);
    ...
    if (isEditMode) {
      ...
      try {
        ...
        const response = await axios.put(
          `${API_BASE_URL}/courses/update/${courseData._id}`,
          formData,
          {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await axios.delete(
        `${API_BASE_URL}/courses/delete/${courseData._id}`
      );
      setCourseData(null);
      setFormData({
        providerId: "",
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await axios.get(
          `${API_BASE_URL}/course-category/categories`
        );
