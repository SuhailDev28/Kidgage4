...
const EditAdvertisementForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(
        `${API_BASE_URL}/advertisement/${editingAdvertisement._id}`,
        formData,
        {
          headers: {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await axios.delete(
        `${API_BASE_URL}/advertisement/${deletingAdvertisementId}`
      );
      fetchAdvertisements();
                  ...
                  {advertisements.map((ad) => (
                    <div key={ad._id} className="poster-box">
                      <div className="poster-img-cont">
                        <img
                          src={`data:image/jpeg;base64,${ad.mobileImage}`}
                          alt={ad.title}
                        />
                        <img
                          src={`data:image/jpeg;base64,${ad.desktopImage}`}
                          alt={ad.title}
                        />
                      </div>
